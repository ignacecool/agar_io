...
function drawCircle(x,y,r,c){
  ...
  if (main_cell_y < half_height_screen){
	...
	/*if(x_distance > distance_x)
		return
	else if(y_distance > distance_y)
		return*/
  ...
  if( (main_cell_y-y)<0 ){
    if ( y_distance < distance_droit_y )
       y = distance_droit_y - y_distance
       draw_y=1
  }

  if((draw_y==0) || (draw_x==0))
    return

	/*if( (main_cell_y-y)>0 )
		y = distance_y - y_distance
	else
		y = distance_y + y_distance*/
...
$("#canvas").on('mousemove', function(event) {
	  MOUSE_X = event.clientX * (SERVER_HEIGHT / HEIGHT)
    MOUSE_Y = event.clientY * (SERVER_WIDTH / WIDTH)
});
